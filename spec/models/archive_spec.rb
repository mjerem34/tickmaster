...
RSpec.describe Archive, type: :model do
  ...
  it { expect(build(:archive, incident_id: @incident.id, sender_id: @user.id, receiver_id: @admin.id)).to be_valid }

  it { expect(build(:archive, incident_id: nil, sender_id: @user.id, receiver_id: @admin.id)).not_to be_valid }

  it { expect(build(:archive, incident_id: @incident.id, sender_id: nil, receiver_id: @admin.id)).not_to be_valid }

  it { expect(build(:archive, incident_id: @incident.id, sender_id: @user.id, receiver_id: nil)).not_to be_valid }
  ...
  it { expect(build(:archive, ip_address_sender: nil, incident_id: @incident.id, sender_id: @user.id, receiver_id: @admin.id)).not_to be_valid }

  it { expect(build(:archive, ip_address_sender: SecureRandom.hex(3), incident_id: @incident.id, sender_id: @user.id, receiver_id: @admin.id)).not_to be_valid }
